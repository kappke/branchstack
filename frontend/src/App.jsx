    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <header className="border-b border-slate-800 bg-slate-900/40">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          <div className="flex items-center gap-2">
            <div className="h-7 w-7 rounded bg-indigo-600 grid place-items-center font-bold">b</div>
            <h1 className="text-lg font-semibold">BranchStack</h1>
            <span className="text-xs text-slate-500">merge branches · dispatch deploys</span>
          </div>
          <div className="flex items-center gap-3">
            {activeToken && (
              <Badge color="green">
                active: {activeToken.name} ({activeToken.label})
              </Badge>
            )}
            {user && <span className="text-xs text-slate-400">signed in as {user.username}</span>}
      ...
      <main className="mx-auto max-w-7xl space-y-4 px-4 py-5">
        ...
        <div className={tab === 'configure' ? 'space-y-4' : 'hidden'}>
          <TokenManager onActiveChange={setActiveToken} />
          <UserManager />
          <ChangePassword />
        </div>
      ...
      <footer className="mx-auto max-w-7xl px-4 py-6 text-center text-xs text-slate-600">
        BranchStack · FastAPI + React + Tailwind · GitHub fine-grained PATs encrypted at rest (SQLite)
      </footer>
    </div>
  )
}